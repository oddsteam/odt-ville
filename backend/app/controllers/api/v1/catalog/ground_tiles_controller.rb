  ...
  module V1
    module Catalog
      class GroundTilesController < BaseController
        # Authoring writes require the `admin` realm role (#100, formerly `editor`
        # in #94 — folded into the single admin role); reads stay open.
        before_action -> { require_role!("admin") }, only: %i[create destroy]

        # GET /api/v1/ground_tiles — the ground-tile catalog. Optional ?type=.
        def index
          scope = ::Catalog::GroundTile.order(:tile_type, :tileset, :row, :col)
          scope = scope.where(tile_type: params[:type]) if params[:type].present?
          render json: scope.map { |t| ::Catalog::GroundTileSerializer.call(t) }
        end
        ...
        def create
          ...
          tile = ::Catalog::GroundTile.find_or_initialize_by(
          ...
          ::Catalog::Terrain.register!(type)

          render json: ::Catalog::GroundTileSerializer.call(tile)
        end

        # DELETE /api/v1/ground_tiles/:id — drop a tile from the catalog.
        def destroy
          ::Catalog::GroundTile.find(params[:id]).destroy!
          head :no_content
        end
      end
    end

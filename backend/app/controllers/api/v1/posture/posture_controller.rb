module Api
  module V1
    module Posture
      ...
      #
      # Leading-colon `::Posture::Client` refs jump to the domain module's client
      # — inside Api::V1::Posture a bare `Posture` would resolve to this namespace.
      class PostureController < BaseController
        ...
        def start
          ...
          session = ::Posture::Client.from_env.start_verification(
        ...
        def sets
          render json: { posture_sets: ::Posture::Client.from_env.list_posture_sets }
        end
        ...
        def confirm
          result = ::Posture::Client.from_env.read_result(session_id: params[:session_id].to_s)
          render json: { granted: ::Posture::Client.granted?(**result), status: result[:status] }
        end

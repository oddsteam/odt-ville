...
class MonsterTest < ActiveSupport::TestCase
  def make(name:, rate:, enabled: true)
    Catalog::Monster.create!(name: name, image: "data:img", encounter_rate: rate, enabled: enabled)
  end

  test "name is required and unique" do
    make(name: "Slime", rate: 1)
    dup = Catalog::Monster.new(name: "Slime", image: "data:img", encounter_rate: 1)
    assert_not dup.valid?
    assert_includes dup.errors[:name], "has already been taken"

    blank = Catalog::Monster.new(image: "data:img", encounter_rate: 1)
    assert_not blank.valid?
  end

  test "encounter_rate must be a non-negative integer" do
    assert_not Catalog::Monster.new(name: "A", image: "i", encounter_rate: -1).valid?
    assert_not Catalog::Monster.new(name: "B", image: "i", encounter_rate: 1.5).valid?
    assert Catalog::Monster.new(name: "C", image: "i", encounter_rate: 0).valid?
  end
  ...
  test "probability is the weighted split across enabled monsters" do
    ...
    total = Catalog::Monster.enabled_rate_total
  ...
  test "an empty or zero-sum pool yields 0 without divide-by-zero" do
    assert_equal 0, Catalog::Monster.enabled_rate_total

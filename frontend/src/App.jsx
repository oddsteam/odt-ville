...
export default function App() {
  ...
  // The community interior + the town are both Phaser scenes inside
  // <VillageGame> — the shell only tracks the *active* community so the
  // GB topbar label can show the right name + session-save knows what
  // to persist. There is no React-level scene routing anymore.
  ...
  const handleEnterCommunity = useCallback((id) => {
    // Phaser's InteriorScene handles the visible transition; the shell
    // just tracks the active id (so the topbar shows the right name)
    // and persists session.
  ...
  const handleExitCommunity = useCallback(
    (idFromBus) => {
      // Phaser's InteriorScene already started the Town scene by the
      // time we get here; this handler just persists session + clears
      // active id so the next reload spawns on the just-exited doormat.
      const id = idFromBus ?? activeCommunityId
      setSession((prev) => ({
        ...(prev || {}),
        last_area: 'town',
  ...
  // Tab switch: opening Admin from inside a community clears the active
  // id so when the user comes back to the village tab the Phaser scene
  // boots into Town (spawning at the last-visited community's doormat
  // via the session).

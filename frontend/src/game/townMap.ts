...
// catalog is the caller's — the live path builds it from the mapped ground
// tiles + persisted terrain priority (#120), the same data the editor reads.
export function bakeTownGround(town: Town, catalog: TileCatalog): BakedGround {
...
export function buildTownMap(
  ...
  const town = buildTown(plotCount, door, footprint, walkMask, edgeMask, policy)
